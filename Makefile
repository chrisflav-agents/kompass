...
ifeq ($(detach), true)
	...
	cd docker/development; USER_ID=$$(id -u) GROUP_ID=$$(id -g) USERNAME=$$(id -un) docker compose exec master bash -c "cd kompass && bash"
else ifeq ($(DEV_CMD),translate)
	cd docker/development; USER_ID=$$(id -u) GROUP_ID=$$(id -g) USERNAME=$$(id -un) docker compose exec master bash -c "cd kompass && python3 manage.py makemessages --locale de --no-location --no-obsolete && python3 manage.py compilemessages"
else ifeq ($(DEV_CMD),createsuperuser)
	cd docker/development; USER_ID=$$(id -u) GROUP_ID=$$(id -g) USERNAME=$$(id -un) docker compose exec master bash -c "cd kompass && python3 manage.py createsuperuser"
else
